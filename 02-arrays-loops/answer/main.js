...
while (notAnswered) {
	userChoise = prompt('Виберіть тему:\n 1. темна чи\n 2. світла')
	switch (userChoise) {
		case '1':
			selectedStyle = 'dark'
			notAnswered = false
...
let header = '<header class = \"' + selectedStyle + '\"><h1>Пірати карибського моря</h1><ul class = "moviesList">'
for (let i = 0; i < movies.length; i++) {
	header += '<li><img src="images\\' + movies[i][1] + '"><br><a href="#' + i + '" class = "' + selectedStyle + 'Link">' + movies[i][0] + '</a></li>'
}
header += '</ul></header>'

var description = '<section class = \"' + selectedStyle + '\"><h2>Опис фільмів</h2>'

for (var i = 0; i < movies.length; i++) {
	description += '<h3 id="' + i + '">' + movies[i][0] + '</h3> <p class="description">' + movies[i][2] + '</p>'
}
description += '</section>'

document.body.innerHTML = header + description